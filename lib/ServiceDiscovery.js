...
const AWSMetaData = require('../utils/AWSMetaData');
...
class ServiceDiscovery {
  constructor(port) {
    ...
    this.amazonDataCenter = process.env.AMAZON_DATACENTER || 'default';
    ...
    this.app.get('/getAWSMetaData', this.getMetaData.bind(this));
  }
  ...
  getMetaData(req,res){
    const data = new AWSMetaData();
    const awsMetaData = (data.getAWSServiceMetaData());
    return res.status(200).json(awsMetaData);
  ...
  getSystemInformation(req,res){
    ...
    if(this.amazonDataCenter !== 'default'){
        const data = new AWSMetaData();
        let awsMetaData = (data.getAWSServiceMetaData());
    }
